...
from log_analyzer.cli import _display_triage, cli
...
def _write_json_logs(filepath, count=10, level="INFO"):
    ...
    with open(filepath, "w") as f:
        for i in range(count):
            f.write(
                json.dumps(
                    {"timestamp": f"2020-01-01T{i % 24:02d}:00:00Z", "level": level, "message": f"Test message {i}"}
                )
                + "\n"
            )
...
class TestAnalyzeCommandIntegration:
    def test_analyze_real_file(self, runner):
        with runner.isolated_filesystem():
            ...
            result = runner.invoke(cli, ["analyze", "test.log", "--no-threading", "--format", "json"])
    ...
    def test_analyze_with_analytics(self, runner):
        with runner.isolated_filesystem():
            _write_json_logs("test.log", 10)
            result = runner.invoke(
                cli, ["analyze", "test.log", "--no-threading", "--format", "json", "--enable-analytics"]
            )
            assert result.exit_code == 0
    ...
    def test_analyze_verbose(self, runner):
        with runner.isolated_filesystem():
            ...
            result = runner.invoke(cli, ["-v", "analyze", "test.log", "--no-threading", "--format", "json"])
            assert result.exit_code == 0
    ...
    def test_detect_json_format(self, runner):
        with runner.isolated_filesystem():
            with open("test.log", "w") as f:
                for i in range(20):
                    f.write(
                        json.dumps({"log": f"message {i}\n", "stream": "stdout", "time": f"2020-01-01T{i:02d}:00:00Z"})
                        + "\n"
                    )
...
class TestAIProviderBase:
    ...
    def test_ai_response_repr(self):
        ...
        resp = AIResponse(content="A" * 100, model="claude-sonnet-4-5-20250929", provider="anthropic", latency_ms=100.0)
        r = repr(resp)
        assert "..." in r
    ...
    def test_ai_response_short_content(self):
        ...
        resp = AIResponse(content="Short", model="test", provider="test")
        r = repr(resp)
        assert "Short" in r
    ...
    def test_triage_result_to_dict(self):
        from log_analyzer.ai_providers.base import Severity, TriageResult
...
class TestParserEdgeCases:
    ...
    def test_aws_text_format(self):
        ...
        parser.parse(line)  # May or may not match — just shouldn't crash

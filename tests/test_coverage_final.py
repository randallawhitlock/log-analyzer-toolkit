...
from log_analyzer.ai_providers.base import (
    AIResponse, ProviderNotAvailableError, AIError,
)
...
class TestAnalyzerMultithreaded:
    def test_multithreaded_analysis(self):
        ...
        try:
            ...
            result = analyzer.analyze(filepath, parser=JSONLogParser(),
                                       use_threading=True, chunk_size=25)
            assert result is not None
    ...
    def test_multithreaded_with_status_codes(self):
        ...
        try:
            analyzer = LogAnalyzer()
            result = analyzer.analyze(filepath, parser=JSONLogParser(),
                                       use_threading=True, chunk_size=10)
            assert result is not None
            assert result.parsed_lines > 0
        finally:
            os.unlink(filepath)
...
class TestAnalyticsEdgeCases:
    def test_compute_analytics_with_entries(self):
        ...
        errors = [
            LogEntry(timestamp=base_time + timedelta(minutes=i*5),
                    level="ERROR", message=f"Error {i}", source=f"mod.{i%3}")
            for i in range(5)
        ]
        warnings_list = [
            LogEntry(timestamp=base_time + timedelta(minutes=i*10),
                    level="WARNING", message=f"Warning {i}", source=f"mod.{i%2}")
            for i in range(3)
        ]
...
class TestParserEdgeCases:
    ...
    def test_all_parsers_can_parse_empty(self):
        ...
        from log_analyzer.parsers import (
            ...
            NginxAccessParser, SyslogParser, JSONLogParser,
        )
        parsers = [
            ApacheAccessParser(), ApacheErrorParser(),
            NginxAccessParser(), SyslogParser(), JSONLogParser(),
